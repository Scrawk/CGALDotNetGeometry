...
namespace CGALDotNetGeometry.Nurbs
{
	...
	public static class NurbsMake
	{
		...
		{
			if (degree < NurbsCurve2d.MIN_DEGREE)
				throw new Exception("The curves degree must be at least " + NurbsCurve2d.MIN_DEGREE);

			if (degree > NurbsCurve2d.MAX_DEGREE)
				throw new Exception("The curves degree must be not be greater than " + NurbsCurve2d.MAX_DEGREE);
